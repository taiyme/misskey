...
import { url, version, locale, lang, updateLocale, commitHash } from '@@/js/config.js';
...
const localeOutdated = localStorage.getItem('localeVersion') !== newLocaleVersion || locale == null;
if (localeOutdated) {
	...
	if (res.status === 200) {
		...
		localStorage.setItem('localeVersion', newLocaleVersion);
