...
<div class="adhpbeos">
	<div class="label" @click="focus">
		<span v-if="$slots.label || counter.ok" class="label-text"><slot name="label"></slot></span>
		<span v-if="counter.ok" class="text-count" :class="{ over: counter.isOver }">{{ counter.remainingChars }}</span>
	</div>
...
watch(v, () => {
	invalid.value = !!inputEl.value?.validity.badInput || (counter.value.ok && counter.value.isOver);
