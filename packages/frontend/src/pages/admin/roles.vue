		...
		<div class="_gaps">
			<MkFolder>
				...
				<div class="_gaps_s">
					...
					<MkFolder v-if="matchQuery([i18n.ts._role._options.tmsAchievementsAvailable, 'canImportUserList'])">
						<template #label>{{ i18n.ts._role._options.tmsAchievementsAvailable }}</template>
						<template #suffix>{{ policies.tmsAchievementsAvailable ? i18n.ts.yes : i18n.ts.no }}</template>
						<MkSwitch v-model="policies.tmsAchievementsAvailable">
							<template #label>{{ i18n.ts.enable }}</template>
						</MkSwitch>
					</MkFolder>
				</div>
