	...
	<template #default>
		<MkSpacer :contentMax="700" :marginMin="16" :marginMax="32">
				...
				<div class="_gaps_m">
					...
					<div inert>
						<MkCodeEditor v-model="customCssRef" readonly lang="css">
							<template #label>{{ i18n.ts._tms._customCss.userCustomCss }}</template>
						</MkCodeEditor>
					</div>
			...
			<template v-else>
				<div class="_gaps_m">
					<div class="_gaps_s">
						<MkInfo warn>{{ i18n.ts._tms._customCss.userCustomCssWarn }}</MkInfo>
						<MkInfo>{{ i18n.ts._tms._customCss.userCustomCssInfo }}</MkInfo>
					</div>
					<div class="_gaps_s">
						...
						<div style="display: flex; gap: 8px; flex-wrap: wrap; align-items: center;">
							<MkSwitch v-model="immediateSaveRef">
								<template #label>
									<span :style="{ color: immediateSaveRef ? 'var(--MI_THEME-error)' : undefined }">
										{{ i18n.ts._tms._customCss.immediateSave }}
									</span>
								</template>
							</MkSwitch>
							<div style="margin-left: auto;">
								<MkButton v-if="immediateSaveRef" danger rounded :disabled="!hasChange" @click="immediatelySave">
									<i class="ti ti-check"></i> {{ i18n.ts.save }}
								</MkButton>
								<MkButton v-else primary rounded :disabled="!hasChange" @click="confirmPreview">
									<i class="ti ti-check"></i> {{ i18n.ts.save }}
								</MkButton>
							</div>
						</div>
...
import { applyUserCustomCss, CUSTOM_CSS_PREVIEW_PARAM_KEY, customCssPreviewData, customCssPreviewId } from '@/tms/custom-css.js';
import MkButton from '@/components/MkButton.vue';
import MkCodeEditor from '@/components/MkCodeEditor.vue';
import MkInfo from '@/components/MkInfo.vue';
import MkSwitch from '@/components/MkSwitch.vue';

const immediateSaveRef = ref(false);

const previewData = customCssPreviewId?.startsWith('user.') ? customCssPreviewData : null;
const previewing = previewData != null;

const customCssRef = ref(previewData ?? miLocalStorage.getItem('customCss') ?? '');
...
async function immediatelySave() {
	...
	miLocalStorage.setItem('customCss', cssText);
	beforeCustomCssRef.value = cssText;
	applyUserCustomCss(cssText);
}
...
async function confirmPreview() {
	...
	const newPreviewId = `user.${window.crypto.randomUUID()}`;
...
definePageMetadata(() => ({
	title: i18n.ts._tms._customCss.userCustomCss,
	icon: 'ti ti-code',
}));
</script>
